...
TEST_F(Test_RawGrpcClient, Test_findStocks)
{
    ...
    // clang-format off
    ASSERT_EQ(status.ok(),                                                 true);
    ASSERT_GE(resp->instruments_size(),                                    1500);
    ASSERT_GE(index,                                                       0);
...
TEST_F(Test_RawGrpcClient, Test_findEtfs)
{
    ...
    // clang-format off
    ASSERT_EQ(status.ok(),                                                 true);
    ASSERT_GE(resp->instruments_size(),                                    275);
    ASSERT_GE(index,                                                       0);
...
TEST_F(Test_RawGrpcClient, Test_findFutures)
{
    ...
    // clang-format off
    ASSERT_EQ(status.ok(),                                                   true);
    ASSERT_GE(resp->instruments_size(),                                      325);
    ASSERT_GE(index,                                                         0);
    ASSERT_EQ(resp->instruments(index).figi(),                               "FUTWHEAT0826");
    ASSERT_EQ(resp->instruments(index).ticker(),                             "W4Q6");
    ...
    ASSERT_EQ(resp->instruments(index).buy_available_flag(),                 true);
    ASSERT_EQ(resp->instruments(index).sell_available_flag(),                true);
    ASSERT_EQ(resp->instruments(index).min_price_increment().units(),        10);
    ASSERT_EQ(resp->instruments(index).min_price_increment().nano(),         0);
    ASSERT_EQ(resp->instruments(index).api_trade_available_flag(),           true);
    ...
    // ASSERT_EQ(resp->instruments(index).initial_margin_on_sell().units(),     2281);
    // ASSERT_EQ(resp->instruments(index).initial_margin_on_sell().nano(),      350000000);
    ASSERT_EQ(resp->instruments(index).min_price_increment_amount().units(), 10);
...
TEST_F(Test_RawGrpcClient, Test_getPortfolio)
{
    ...
    ASSERT_EQ(resp->positions(0).instrument_uid(),     RUBLE_UID);
    // clang-format on
}
