...
TEST_F(Test_BuyDecision3, Test_makeDecision)
{
    ...
    QString cause =
        buyDecision3->makeDecision(QThread::currentThread(), &configMock, 0, &stock, false, -1, false, 100.0f, -1.0f, 0.04f);
    ...
    cause = buyDecision3->makeDecision(
    ...
    cause = buyDecision3->makeDecision(QThread::currentThread(), &configMock, 0, &stock, true, 14, false, 100.0f, -1.0f, 0.04f);
    ...
    cause = buyDecision3->makeDecision(QThread::currentThread(), &configMock, 0, &stock, false, -1, false, 100.0f, -1.0f, 0.04f);
    ...
    ASSERT_EQ(
        cause,
        QString(
            "Decided to buy because the price fall to 100.00 \u20BD from 150.00 \u20BD at %1 within last 10 days and the "
            "fall is -33.33%"
        )
...
TEST_F(Test_BuyDecision3, Test_asapMode)
{
    ASSERT_EQ(buyDecision3->asapMode(), ASAP_MODE_NONE);
}
