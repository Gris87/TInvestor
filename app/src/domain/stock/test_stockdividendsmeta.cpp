...
TEST_F(Test_StockDividendsMeta, Test_copy_constructor)
{
    ...
    ASSERT_NEAR(stockDividendsMeta2.yield,          3.0f, 0.0001f);
    // clang-format on
}
...
TEST_F(Test_StockDividendsMeta, Test_fromJsonObject)
{
    ...
    const QString content = R"({"createTimestamp":1,"paymentTimestamp":2,"yield":"3.00"})";
    ...
    ASSERT_NEAR(stockDividendsMeta.yield,          3.0f, 0.0001f);
    // clang-format on
...
TEST_F(Test_StockDividendsMeta, Test_toJsonObject)
{
    ...
    const QString expectedContent = R"({"createTimestamp":1,"paymentTimestamp":2,"yield":"3.00"})";
...
TEST_F(Test_StockDividendsMeta, Test_equals)
{
    ...
    stockDividendsMeta2.yield = 3000.0f;
    ASSERT_NE(stockDividendsMeta, stockDividendsMeta2);
    stockDividendsMeta2.yield = 3.0f;
    ASSERT_EQ(stockDividendsMeta, stockDividendsMeta2);
}
