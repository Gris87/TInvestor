...
bool operator==(const StockTableEntry& lhs, const StockTableEntry& rhs)
{
    return lhs.instrumentId == rhs.instrumentId && lhs.instrumentTicker == rhs.instrumentTicker &&
           ...
           lhs.turnover == rhs.turnover && qAbs(lhs.payback - rhs.payback) < FLOAT_EPSILON && lhs.dividends == rhs.dividends &&
           lhs.shorts == rhs.shorts && qAbs(lhs.dayStartPrice - rhs.dayStartPrice) < FLOAT_EPSILON &&
