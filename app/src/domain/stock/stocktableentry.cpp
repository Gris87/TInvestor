...
bool operator==(const StockTableEntry& lhs, const StockTableEntry& rhs)
{
    return lhs.instrumentId == rhs.instrumentId && lhs.instrumentTicker == rhs.instrumentTicker &&
           ...
           lhs.locked == rhs.locked && qAbs(lhs.price - rhs.price) < FLOAT_EPSILON && qAbs(lhs.dayChange - rhs.dayChange) < FLOAT_EPSILON &&
           qAbs(lhs.dateChange - rhs.dateChange) < FLOAT_EPSILON && lhs.turnover == rhs.turnover &&
           qAbs(lhs.payback - rhs.payback) < FLOAT_EPSILON && qAbs(lhs.dayStartPrice - rhs.dayStartPrice) < FLOAT_EPSILON &&
