...
constexpr float PRICE_FALL_DEFAULT_30 = 3.5f;
constexpr float LOSE_YIELD_DEFAULT_30 = 0.3f;
constexpr int   DURATION_DEFAULT_30   = 60;

// For commission 0.05%
constexpr bool  ENABLED_DEFAULT_5    = false;
constexpr float PRICE_FALL_DEFAULT_5 = 2.5f;
constexpr float LOSE_YIELD_DEFAULT_5 = 0.3f;
constexpr int   DURATION_DEFAULT_5   = 60;

// For commission 0.04%
constexpr bool  ENABLED_DEFAULT_4    = false;
constexpr float PRICE_FALL_DEFAULT_4 = 2.5f;
constexpr float LOSE_YIELD_DEFAULT_4 = 0.3f;
...
void BuyDecision2Config::makeDefault(int commissionInt)
{
    const QWriteLocker lock(mRwMutex);
...
bool BuyDecision2Config::isDefault(int commissionInt)
{
    BuyDecision2Config defaultConfig;
    defaultConfig.makeDefault(commissionInt);

    const QReadLocker lock(mRwMutex);

    return mEnabled == defaultConfig.mEnabled && mPriceFall == defaultConfig.mPriceFall &&
           mLoseYield == defaultConfig.mLoseYield && mDuration == defaultConfig.mDuration;
}
...
QStringList BuyDecision2Config::variantsAsJson() const
{
    ...
    const QStringList priceFallVariants = {"3.50", "4.00", "4.50", "5.00"};
    const QStringList loseYieldVariants = {"0.3", "0.5", "0.7"};
    const QStringList durationVariants  = {"5", "15", "30", "60"};
