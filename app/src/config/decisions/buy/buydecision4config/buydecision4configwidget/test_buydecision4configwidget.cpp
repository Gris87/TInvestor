...
class Test_BuyDecision4ConfigWidget : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        configWidget = new BuyDecision4ConfigWidget(buyDecision4ConfigMock);
    }
    ...
    BuyDecision4ConfigWidget*           configWidget;
    StrictMock<BuyDecision4ConfigMock>* buyDecision4ConfigMock;
};



TEST_F(Test_BuyDecision4ConfigWidget, Test_constructor_and_destructor)
{
}

TEST_F(Test_BuyDecision4ConfigWidget, Test_updateUiFromConfig)
{
...
TEST_F(Test_BuyDecision4ConfigWidget, Test_makeReadOnly)
{
    // clang-format off
...
TEST_F(Test_BuyDecision4ConfigWidget, Test_on_enabledCheckBox_checkStateChanged)
{
    const InSequence seq;
...
TEST_F(Test_BuyDecision4ConfigWidget, Test_on_priceFallDoubleSpinBox_valueChanged)
{
    const InSequence seq;
...
TEST_F(Test_BuyDecision4ConfigWidget, Test_on_loseYieldDoubleSpinBox_valueChanged)
{
    const InSequence seq;
...
TEST_F(Test_BuyDecision4ConfigWidget, Test_on_durationSpinBox_valueChanged)
{
    const InSequence seq;
    ...
    EXPECT_CALL(*buyDecision4ConfigMock, setDuration(2));
    configWidget->ui->durationSpinBox->setValue(2);

    EXPECT_CALL(*buyDecision4ConfigMock, setDuration(3));
