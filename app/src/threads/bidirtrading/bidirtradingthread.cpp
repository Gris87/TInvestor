...
Quotation BiDirTradingThread::calculateBuyPriceInternal(const tinkoff::GetOrderBookResponse& tinkoffOrderBook, float spread)
{
    ...
    const double maximumBuyPrice = basePrice * (1 - ((spread + ADDITIONAL_GAP_PERCENT) / HUNDRED_PERCENT));
...
)
{
    ...
    const qint64 coef = static_cast<qint64>(std::ceil(res / quotationToDouble(mMinPriceIncrement)));
