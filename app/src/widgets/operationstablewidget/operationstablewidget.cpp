...
// TODO: Update widths
#ifdef Q_OS_WINDOWS
const int COLUMN_WIDTHS[OPERATIONS_COLUMN_COUNT] = {110, 133, 309, 56, 106, 95, 167, 87, 85, 95, 173, 194, 10, 10, 115, 95};
#else
const int COLUMN_WIDTHS[OPERATIONS_COLUMN_COUNT] = {121, 138, 154, 59, 114, 102, 180, 95, 90, 101, 184, 206, 10, 10, 125, 101};
#endif
...
void OperationsTableWidget::saveWindowState(const QString& type)
{
    ...
    mSettingsEditor->setValue(type + "/columnWidth_TotalYieldWithCommission",        ui->tableWidget->columnWidth(OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_COLUMN));
    mSettingsEditor->setValue(type + "/columnWidth_TotalYieldWithCommissionPercent", ui->tableWidget->columnWidth(OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_PERCENT_COLUMN));
...
void OperationsTableWidget::loadWindowState(const QString& type)
{
    ...
    ui->tableWidget->setColumnWidth(OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_COLUMN,         mSettingsEditor->value(type + "/columnWidth_TotalYieldWithCommission",        COLUMN_WIDTHS[OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_COLUMN]).toInt());
    ui->tableWidget->setColumnWidth(OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_PERCENT_COLUMN, mSettingsEditor->value(type + "/columnWidth_TotalYieldWithCommissionPercent", COLUMN_WIDTHS[OPERATIONS_TOTAL_YIELD_WITH_COMMISSION_PERCENT_COLUMN]).toInt());
