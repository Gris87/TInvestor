...
void Workbook::saveToXmlFile(QIODevice* device) const
{
    ...
    for (int i = 0; i < d->sheets.size(); ++i)
    {
        ...
        if (sheet->sheetType() == AbstractSheet::ST_WorkSheet)
        {
            d->relationships->addDocumentRelationship(
                QStringLiteral("/worksheet"), QStringLiteral("worksheets/sheet%1.xml").arg(++worksheetIndex)
            );
        }
        else
        {
            d->relationships->addDocumentRelationship(
                QStringLiteral("/chartsheet"), QStringLiteral("chartsheets/sheet%1.xml").arg(++chartsheetIndex)
            );
        }
    ...
    if (!d->externalLinks.isEmpty())
    {
        ...
        for (int i = 0; i < d->externalLinks.size(); ++i)
        {
            ...
            d->relationships->addDocumentRelationship(
                QStringLiteral("/externalLink"), QStringLiteral("externalLinks/externalLink%1.xml").arg(i + 1)
            );
            writer.writeAttribute(QStringLiteral("r:id"), QStringLiteral("rId%1").arg(d->relationships->count()));
        }
