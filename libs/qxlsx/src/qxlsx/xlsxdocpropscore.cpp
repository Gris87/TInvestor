...
void DocPropsCore::saveToXmlFile(QIODevice* device) const
{
    ...
    writer.writeTextElement(
        dc, QStringLiteral("creator"), it != m_properties.constEnd() ? it.value() : QStringLiteral("Qt Xlsx Library")
    );
    ...
    it = m_properties.constFind(QStringLiteral("creator"));
    writer.writeTextElement(
        cp, QStringLiteral("lastModifiedBy"), it != m_properties.constEnd() ? it.value() : QStringLiteral("Qt Xlsx Library")
    );
