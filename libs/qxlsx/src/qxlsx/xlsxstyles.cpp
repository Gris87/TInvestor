...
#include "src/qxlsx/xlsxcolor_p.h"
...
void Styles::fixNumFmt(const Format& format)
{
    ...
    if (format.hasProperty(FormatPrivate::P_NumFmt_Id) && !format.stringProperty(FormatPrivate::P_NumFmt_FormatCode).isEmpty())
    {
        return;
    }
...
void Styles::writeFont(QXmlStreamWriter& writer, const Format& format, bool isDxf) const
{
    ...
    if (format.hasProperty(FormatPrivate::P_Font_Condense) && !format.boolProperty(FormatPrivate::P_Font_Condense))
    {
        writer.writeEmptyElement(QStringLiteral("condense"));
        writer.writeAttribute(QStringLiteral("val"), QStringLiteral("0"));
    }
    if (format.hasProperty(FormatPrivate::P_Font_Extend) && !format.boolProperty(FormatPrivate::P_Font_Extend))
    {
        ...
        if (reader.tokenType() == QXmlStreamReader::StartElement)
        {
            if (reader.name() == QLatin1String("patternFill"))
            {
                ...
                if (attributes.hasAttribute(QLatin1String("patternType")))
                {
                    ...
                    while (!reader.atEnd() &&
                           !(reader.tokenType() == QXmlStreamReader::EndElement && reader.name() == QLatin1String("patternFill")))
                    {
                        reader.readNextStartElement();
                        if (reader.tokenType() == QXmlStreamReader::StartElement)
                        {
...
bool Styles::readIndexedColors(QXmlStreamReader& reader)
{
    ...
    while (!reader.atEnd() &&
           !(reader.name() == QLatin1String("indexedColors") && reader.tokenType() == QXmlStreamReader::EndElement))
    {
        reader.readNextStartElement();
        if (reader.tokenType() == QXmlStreamReader::StartElement)
        {
