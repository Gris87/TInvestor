...
{
    addOverride(
        QStringLiteral("/xl/worksheets/%1.xml").arg(name), m_document_prefix + QLatin1String("spreadsheetml.worksheet+xml")
    );
}
...
{
    addOverride(
        QStringLiteral("/xl/chartsheets/%1.xml").arg(name), m_document_prefix + QLatin1String("spreadsheetml.chartsheet+xml")
    );
}
...
{
    addOverride(
        QStringLiteral("/xl/externalLinks/%1.xml").arg(name), m_document_prefix + QLatin1String("spreadsheetml.externalLink+xml")
    );
}
...
void ContentTypes::saveToXmlFile(QIODevice* device) const
{
    ...
    writer.writeAttribute(
        QStringLiteral("xmlns"), QStringLiteral("http://schemas.openxmlformats.org/package/2006/content-types")
    );
