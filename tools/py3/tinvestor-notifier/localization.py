...
msg_cron_failure_found = "⚠ Внимание! Обнаружен сбой задачи cron. Пожалуйста, проведите анализ, используя созданный {cron_failure_path} файл."
...
msg_dividends = "⚠ Внимание! Объявлены дивиденды для {ticker}({name}) с доходностью {yield_value}% и датой отсечки {date_str}"
...
recommend_to_short_regexp = re.compile(r".*(арест|задержа|заблокирова|банкротств| суд([^аноья]|$)| иск([^лору]|$)|изъят|сизо|упк рф|долг([^о]|$)|погиб|убит|умер([^е]|$)|уничтожен|взорван|взрыв|разрушен|допк|эмиссия).*")
