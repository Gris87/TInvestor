...
TEST_F(Test_AutorunEnabler, Test_setEnabled)
{
    ...
    EXPECT_CALL(
        *autorunSettingsEditorMock,
        setValue(QString("CurrentVersion/Run/TInvestorNotifier"), QVariant(QString("\"%1\" --autorun").arg(appPath)))
    );
    ...
    EXPECT_CALL(*autorunSettingsEditorMock, remove(QString("CurrentVersion/Run/TInvestorNotifier")));
...
TEST_F(Test_AutorunEnabler, Test_setEnabled)
{
    ...
    EXPECT_CALL(*fileFactoryMock, newInstance(QDir::homePath() + "/.config/autostart/TInvestorNotifier.desktop"))
